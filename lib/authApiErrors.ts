...
function isDuplicateEmailError(error: DataApiRequestError) {
  ...
  return details?.code === '23505' && (text.includes('email') || constraint.includes('email'))
}
...
export function toSafeAuthError(error: unknown): PublicError {
  if (error instanceof DataApiRequestError) {
    if (isDuplicateEmailError(error)) return { status: 409, message: 'Este e-mail já está cadastrado.' }
    if (isDuplicateNameError(error)) return { status: 409, message: 'Este nome de usuário já está em uso.' }

    const isConfigOrPermissionIssue = error.status === 401 || error.status === 403
    if (isConfigOrPermissionIssue) {
      return { status: 500, message: 'Configuração de autenticação indisponível no momento. Tente novamente em instantes.' }
    }

    return { status: 503, message: 'Serviço de autenticação indisponível no momento. Tente novamente em instantes.' }
  }
  ...
  if (error instanceof Error) {
    ...
    if (message.includes('column') && message.includes('does not exist')) {
      return { status: 500, message: 'A estrutura da tabela de usuários está incompatível com a autenticação. Verifique as migrations.' }
    }
