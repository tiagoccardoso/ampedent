...
export async function getAdminByEmail(email: string) {
  const rows = await sql`SELECT id, email, name, role, password_hash FROM admin_users WHERE email = ${email} LIMIT 1`
...
export async function createAdminUser(email: string, name: string, role: AdminRole, passwordHash: string) {
  const rows = await sql`INSERT INTO admin_users (email, name, role, password_hash) VALUES (${email}, ${name}, ${role}, ${passwordHash}) RETURNING id, email, name, role`
...
export async function listAdminUsers() {
  const rows = await sql`SELECT id, email, name, role FROM admin_users ORDER BY created_at ASC`
...
export async function deleteAdminById(id: string) { await sql`DELETE FROM admin_users WHERE id = ${id}` }
