...
**Tailwind CSS**, **Neon Postgres** e sessão administrativa própria baseada em cookie assinado. Ele conta com um sistema intuitivo de
...
- **Agendamento de consultas:** usuários podem agendar consultas facilmente pelo
  ...
  Postgres (Neon) para facilitar o gerenciamento.
...
- **Tailwind CSS**: framework CSS utility-first para criar designs personalizados
  com rapidez.
- **Neon Postgres**: banco de dados PostgreSQL hospedado usado para armazenar
...
- Adicione as variáveis de ambiente necessárias para acesso ao Postgres via API e autenticação local por sessão.
  ...
  # Conexão PostgreSQL do Neon (obrigatória)
  ...
  # Endpoint HTTP da Data API usado pelas rotas legacy
  NEON_AUTH_BASE_URL=https://<seu-endpoint-data-api>
...
5. **Execute os SQLs em `db/migrations/` no banco Neon e, em
...
Depois da migração, execute `node createuser.js` (ou atualize `admin_users.password_hash`) para habilitar o login administrativo por email e senha.
