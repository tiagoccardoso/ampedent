...
async function saveProfessional(formData: FormData) {
  ...
  try {
    if (!fullName) target += '?error=Informe+o+nome+do+profissional'
      ...
      const updated = await sql`update professionals set full_name=${fullName}, specialty=${optional(formData.get('specialty'))}, cro=${optional(formData.get('cro'))}, schedule_notes=${optional(formData.get('schedule_notes'))}, is_active=${formData.get('is_active') === 'on'} where id=${id}::uuid returning id`
      target += (updated as unknown[]).length ? '?ok=Profissional+atualizado+com+sucesso' : '?error=Profissional+n%C3%A3o+encontrado'
    } else {
      await sql`insert into professionals (full_name, specialty, cro, schedule_notes, is_active) values (${fullName}, ${optional(formData.get('specialty'))}, ${optional(formData.get('cro'))}, ${optional(formData.get('schedule_notes'))}, ${formData.get('is_active') === 'on'})`
  ...
  try {
    ...
    if (!professionalId || Number.isNaN(weekday) || !startTime || !endTime) target += '?error=Preencha+os+campos+obrigat%C3%B3rios+da+agenda'
      ...
      await sql`
        ...
        on conflict (professional_id, weekday) do update set start_time=excluded.start_time, end_time=excluded.end_time, break_start=excluded.break_start, break_end=excluded.break_end, appointment_duration_minutes=excluded.appointment_duration_minutes, is_active=excluded.is_active
      `
