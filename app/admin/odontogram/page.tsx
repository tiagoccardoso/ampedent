...
async function saveOdontogramEntry(formData: FormData) {
  ...
  const admin = await assertOdontogramAccess()
  ...
  try {
    if (!uuidPattern.test(patientId) || !toothCodeSet.has(toothCode) || (!plannedProcedure && !procedureId)) {
      ...
      const patientRows = await sql`select id from patients where id = ${patientId}::uuid limit 1`
      const procedureRows = procedureId
        ? await sql`select id from procedures where id = ${procedureId}::uuid and is_active = true limit 1`
        : []
...
async function deleteOdontogramEntry(formData: FormData) {
  ...
  try {
    if (!uuidPattern.test(patientId) || !uuidPattern.test(entryId)) {
      ...
      const deleted = await sql`delete from odontogram_entries where id = ${entryId}::uuid and patient_id = ${patientId}::uuid returning id`
...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string, string>> }) {
  ...
  const [entries, patients, procedures] = await Promise.all([
    sql`
      ...
      left join procedures pr on pr.id = o.procedure_id
      where (${selectedPatientId || null}::uuid is null or o.patient_id = ${selectedPatientId || null}::uuid)
      order by o.updated_at desc, o.created_at desc
      limit 200
    `,
    sql`select id, full_name from patients where status = 'active' order by full_name`,
