...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string,string>> }) {
  ...
  return <section className='space-y-4'><h1 className='text-2xl font-bold'>Pacientes</h1><FormFeedback ok={params.ok} error={params.error} /><form action={savePatient} className='grid md:grid-cols-2 gap-3'><input name='full_name' placeholder='full_name *' required /><input name='phone' placeholder='phone *' required /><input name='cpf' placeholder='cpf' /><input name='birth_date' type='date' /><input name='email' type='email' placeholder='email' /><input name='guardian_name' placeholder='guardian_name' /><input name='address' className='md:col-span-2' placeholder='address' /><textarea name='notes' className='md:col-span-2' placeholder='notes' /><select name='status'>{patientStatus.map(s => <option key={s} value={s}>{s}</option>)}</select><SubmitButton /></form><div className='overflow-auto'><table className='w-full text-sm'><thead><tr><th>Nome</th><th>Telefone</th><th>Status</th></tr></thead><tbody>{(patients as any[]).map(p => <tr key={p.id}><td>{p.full_name}</td><td>{p.phone}</td><td>{p.status}</td></tr>)}</tbody></table></div></section>
}
